...
def classify_wood_leaf_point_cloud( point_cloud,
    ...
    print("Length Total", orig_len)
    print("wood_indices", wood_indices.shape)
    print("leaf_indices", leaf_indices.shape)

    wood_mask = np.zeros(orig_len, dtype=bool)
    if len(wood_indices) > 0:
        wood_mask[wood_indices.astype(int)] = True

    leaf_mask = np.zeros(orig_len, dtype=bool)
    if len(leaf_indices) > 0:
        leaf_mask[leaf_indices.astype(int)] = True
