...
class ArbaroGenerator(BaseTreeGenerator):
    ...
    def generate_tree(self, seed: int, height: float, out_obj: str, out_json: str, **kwargs):
        ...
        # Arbaro outputs Y-up; convert to Z-up.
        ...
        if temp_xml_leaf.exists():
            os.remove(temp_xml_leaf)
        if temp_xml_noleaf.exists():
            os.remove(temp_xml_noleaf)

        extract_cylinders(str(noleaf_obj), out_json)
