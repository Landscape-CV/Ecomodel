...
def generate_from_geometry_nodes(seed, out_obj, out_json):
    """
    Generates a tree by randomizing parameters of an existing 'Mangrove tree' object 
    built with Geometry Nodes, converting it to a mesh, and decimating it for optimization.
    
    Args:
        seed (int): Random seed to perturb geometry node parameters.
        out_obj (str): Filepath to save the exported .obj model.
    ...
    # We mutate integer inputs as they usually represent Random Seeds within the Node graph
    for mod in obj.modifiers:
        if mod.type == 'NODES':
            for k in mod.keys():
                # Filter out standard UI/RNA keys and boolean attribute toggles, keep integer seeds
    ...
    # Convert Geometry Nodes output (with all its instances like leaves/branches) into a single raw MESH
