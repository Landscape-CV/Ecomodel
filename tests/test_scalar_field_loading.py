"""
Regression tests for the scalar-field selector in
``Utils.Utils.load_point_cloud``.

RIEGL-style terrestrial exports often leave the standard ``intensity`` field
at zero and put return strength in an extra ``Reflectance`` dimension.  The
loader can map any LAS dimension to the working intensity column and
optionally rescale it to 0-65535 so negative/fractional units (reflectance in
dB) survive the pipeline's positive intensity thresholds.

These tests build a tiny synthetic LAS fixture so they run without the real
...
from Utils.Utils import load_point_cloud, list_las_scalar_fields


def _write_fixture(path: Path, n: int = 200) -> None:
    """Write a small LAS (point format 6) with all-zero intensity and a
    negative-valued ``Reflectance`` extra dimension, mirroring the real
    mangrove file."""
    ...
    las.intensity = np.zeros(n, dtype=np.uint16)            # all-zero, like the real file
...
def test_reflectance_normalized_to_intensity_range(las_file):
    ...
    # The whole cloud should survive (loader uses >= 0); nothing wiped.
    assert len(pd) == 200
